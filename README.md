...
**V1 基线策略未通过决策门**（见 DESIGN §14）：在 2021–2026 真实数据上，24 组参数全部跑输沪深300 buy&hold。这是系统**如实证伪**——回测器/决策门正确履职。

**因此目前只能用 SHADOW（纸盘）模式**：读报告、跟踪纸面业绩、**不要投入真金白银**，直到你迭代策略（见下文"调优"）通过决策门。
