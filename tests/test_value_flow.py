...
def test_check_exits_trailing_triggered():
    """Price drops > trailing distance from peak → exit."""
    n = 100
    ...
    shares = pd.Series(np.linspace(50, 100, n), dtype=float, index=close.index)
    positions = {"X": {"entry_date": close.index[0]}}
    ctx = {"close": {"X": close}, "share": {"X": shares}}
    # peak=15, last=11, ATR~? with min_hold=60 → should trigger (price dropped significantly)
    exits = vf.check_exits(positions, ctx, {**PARAMS, "rotation": {"value_flow": {"min_hold_days": 0}}})
    assert "X" in exits
...
def test_check_exits_no_exit_when_rising():
    ...
    ctx = {"close": {"X": close}, "share": {"X": shares}}
    exits = vf.check_exits(positions, ctx, {**PARAMS, "rotation": {"value_flow": {"min_hold_days": 0}}})
    assert "X" not in exits  # price rising, trailing follows up, no exit
